...
dnf install --best --allowerasing -y arj awesome-vim-colorschemes azure-cli cabextract code @development-tools dnf-utils dpkg dragon elisa-player fprintd-devel gimp gimp-data-extras gimp-*-plugin gimp-elsamuko gimp-*-filter gimp-help gimp-help-es gimp-layer* gimp-lensfun gimp-*-masks gimp-resynthesizer gimp-save-for-web gimp-separate+ gimp-*-studio gimp-wavelet* gimpfx-foundry gwenview go htop hunspell-es info innoextract kamoso kate kcalc kdiskmark kget kommit konversation ksystemlog kubernetes-client lha libcurl-devel libfprint-devel libreoffice-draw libreoffice-langpack-es libreoffice-help-es libxml2-devel lzma mozilla-ublock-origin neofetch nodejs-bash-language-server okular perl pstoedit pycharm-community pycharm-community-doc pycharm-community-plugins redhat-lsb-core slack thunderbird unace unrar wireshark xkill
